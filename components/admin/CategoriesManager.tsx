...
export default function CategoriesManager({ initialCategories, initialMarques = [] }: Props) {
  ...
  function openMarqueCreate() {
    setMarqueForm(empty);
    setEditingMarque(null);
    setMarqueError("");
    setMarqueModal("create");
  }

  function openMarqueEdit(m: AdminMarque) {
    setMarqueForm({ nom: m.nom, description: m.description });
    setEditingMarque(m);
    setMarqueError("");
    setMarqueModal("edit");
  }
    ...
    try {
      if (marqueModal === "create") {
        const res = await fetch("/api/admin/marques", {
          ...
          body: JSON.stringify({ nom: marqueForm.nom.trim(), description: marqueForm.description.trim() }),
        });
        const data = await res.json();
        if (!res.ok) throw new Error(data.error);
        const newM: AdminMarque = { id: data.id, nom: marqueForm.nom.trim(), description: marqueForm.description.trim(), nb_produits: 0 };
        setMarques(l => [...l, newM].sort((a, b) => a.nom.localeCompare(b.nom)));
      } else if (editingMarque) {
        const res = await fetch(`/api/admin/marques/${editingMarque.id}`, {
          method: "PUT",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ nom: marqueForm.nom.trim(), description: marqueForm.description.trim() }),
        });
        ...
        if (!res.ok) throw new Error(data.error);
        setMarques(l =>
          l.map(m => m.id === editingMarque.id
            ? { ...m, nom: marqueForm.nom.trim(), description: marqueForm.description.trim() }
            : m
      ...
      setMarqueError(err instanceof Error ? err.message : "Erreur");
    } finally {
      setMarqueSaving(false);
    }
  }
    ...
    <div className="space-y-6">
      ...
      <div className="mt-8 space-y-4">
        <div className="flex items-center justify-between">
          <div>
            <h2 className="font-display font-700 text-lg text-slate-900">Marques</h2>
            <p className="text-xs text-slate-400 mt-0.5">Organisez vos produits par marque</p>
          </div>
          <button
            onClick={openMarqueCreate}
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <div className="bg-white rounded-2xl border border-slate-100 p-5 shadow-sm">
            <div className="flex items-start justify-between mb-3">
              <p className="text-xs font-bold uppercase tracking-wide text-slate-400">Produits avec marque</p>
              <Package className="w-8 h-8 text-slate-400 opacity-20" />
            </div>
            <p className="text-2xl font-bold text-slate-900 tabular-nums">
              {marques.reduce((s, m) => s + m.nb_produits, 0)}
            </p>
          </div>
        </div>
            ...
            <div className="space-y-4">
              <div>
                ...
                <input
                  ...
                  onChange={e => setMarqueForm(f => ({ ...f, nom: e.target.value }))}
                  placeholder="Ex: Samsung"
                  autoFocus
                  onKeyDown={e => e.key === "Enter" && handleMarqueSave()}
                  className={inputCls}
                />
              </div>
              <div>
                ...
                <textarea
                  value={marqueForm.description}
                  onChange={e => setMarqueForm(f => ({ ...f, description: e.target.value }))}
