...
router.post("/api/admin/entrepots", async (req, res) => {
  ...
  try {
    const newId = await upsertEntrepot({ id: id ? Number(id) : undefined, nom: nom.trim(), telephone: telephone || null, adresse: adresse || null, notes: notes || null, actif: actif !== false });
