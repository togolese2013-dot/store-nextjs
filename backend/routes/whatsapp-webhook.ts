...
router.get("/api/webhooks/whatsapp", async (req, res) => {
  ...
  const verifyToken = (await getSetting("wa_webhook_verify_token")) || FALLBACK_VERIFY_TOKEN;
...
router.post("/api/webhooks/whatsapp", async (req, res) => {
  ...
  try {
    ...
    // Same Meta app receives webhooks for other WhatsApp Business Accounts on this
    // Business Manager (e.g. "Hamid Cargo") — ignore anything not addressed to our number.
