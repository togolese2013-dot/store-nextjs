...
import { ArrowLeft, PackageMinus, CheckCircle2, AlertCircle, Search, X } from "lucide-react";
        ...
        {success ? (
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div>
              ...
              {selected ? (
                <div className="flex items-center justify-between gap-3 px-4 py-3 rounded-2xl border-2 border-red-300 bg-red-50">
                  <div>
                    <p className="font-semibold text-slate-800 text-sm">{selected.nom}</p>
                    <p className={`text-xs font-semibold mt-0.5 ${
                      selected.stock_boutique === 0 ? "text-red-500" :
                      selected.stock_boutique <= 5  ? "text-amber-500" : "text-green-600"
                    }`}>
                      {selected.reference} — stock disponible : {selected.stock_boutique} unité(s)
                    </p>
                  </div>
                  <button type="button" onClick={() => { setSelected(null); setForm(f => ({ ...f, quantite: "" })); }}
                    className="p-1.5 rounded-xl hover:bg-red-100 text-slate-400 hover:text-slate-700 transition-colors">
                    <X className="w-4 h-4" />
                  </button>
                </div>
              ) : (
                        ...
                        <li key={p.id}>
                          <button type="button" onClick={() => selectProduit(p)}
                            ...
                            <span className={`ml-2 text-xs font-semibold ${
                              p.stock_boutique === 0 ? "text-red-500" :
                              p.stock_boutique <= 5  ? "text-amber-500" : "text-green-600"
                            }`}>stock: {p.stock_boutique}</span>
                          </button>
                        </li>
                      ))}
                    </ul>
            ...
            <div>
              ...
              <input
                ...
                required
                min={1}
                max={selected?.stock_boutique ?? undefined}
                value={form.quantite}
